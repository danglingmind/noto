...
### 7.2 Snapshot upload (two-step flow)

Pages with embedded images/videos can exceed 10 MB. Vercel serverless functions have a hard 4.5 MB request-body limit, so HTML is **never** sent through the Next.js API. Instead the extension uses a two-step flow: get a signed Supabase storage URL, upload directly, then commit.

---

#### Step A — `POST /api/extension/snapshot/prepare`

Validates access, creates a `PENDING` file/revision record, and returns a signed Supabase upload URL. The signed URL is valid for **2 minutes**.
...
{
  "fileId": "file_yyy",
  "uploadUrl": "https://xxx.supabase.co/storage/v1/object/upload/sign/files/snapshots/..."
}
```

---

#### Step B — upload HTML directly to Supabase

```ts
await fetch(uploadUrl, {
  method: 'PUT',
  headers: { 'Content-Type': 'text/html' },
  body: htmlContent          // raw string, any size — bypasses Vercel entirely
})
```

---

#### Step C — `POST /api/extension/snapshot/commit`

Called after the direct upload succeeds. Updates the file to `READY` and returns the full file record.

**Request body**
```json
{
  "fileId": "file_yyy",     // returned by /prepare
  "fileSize": 2097152       // optional — byte length of the HTML string
}
```
...
| Status | Body | Meaning |
|---|---|---|
| 400 | `Missing required fields` | `title` or `url` absent |
...
| 404 | `User not found` | |
| 500 | `Failed to create upload URL` | Supabase signed URL error |

**Error cases — /commit**
| Status | Body | Meaning |
|---|---|---|
| 400 | `Missing required field: fileId` | `fileId` absent |
| 401 | `Unauthorized` | No/invalid token |
| 403 | `File not found or access denied` | User lacks EDITOR+ role on the file |
| 404 | `File not found` | `fileId` doesn't exist |
| 409 | `File is not in PENDING state` | Already committed or failed |
...
export async function prepareSnapshot(token: string, payload: Omit<SnapshotPayload, 'htmlContent'>) {
  return apiFetch<{ fileId: string; uploadUrl: string }>(
    '/api/extension/snapshot/prepare', token, { method: 'POST', body: JSON.stringify(payload) }
  )
}

export async function uploadSnapshotHtml(uploadUrl: string, htmlContent: string): Promise<void> {
  const res = await fetch(uploadUrl, {
    method: 'PUT',
    headers: { 'Content-Type': 'text/html' },
    body: htmlContent
  })
  if (!res.ok) throw new ApiError(res.status, 'Direct upload to storage failed')
}

export async function commitSnapshot(token: string, fileId: string, fileSize: number) {
  return apiFetch<{ success: true; file: VynlFile }>(
    '/api/extension/snapshot/commit', token,
    { method: 'POST', body: JSON.stringify({ fileId, fileSize }) }
  )
}

// Usage in background.ts:
// const { fileId, uploadUrl } = await prepareSnapshot(token, { ...payload })
// await uploadSnapshotHtml(uploadUrl, htmlContent)
// const { file } = await commitSnapshot(token, fileId, htmlContent.length)
...
```
https://vynl.in/legal/extension-privacy
```
This page is live on the main app and covers every permission in the manifest. Use this URL in both the Chrome Web Store Developer Dashboard and the Firefox Add-ons submission form.

**Chrome Web Store — key fields:**
- Category: `Productivity`
- Single purpose description: *"Capture a webpage snapshot from your active tab and upload it to a Vynl project for annotation and feedback."*
- Privacy policy URL: `https://vynl.in/legal/extension-privacy`
- Permissions justification (required in the submission form — copy from §2 of the policy page): each permission maps 1:1 to a row in the table on that page
- Remote code execution: **No** — all logic is bundled; no `eval`, no remotely fetched scripts

**Firefox Add-ons — key fields:**
- License: Proprietary
- Privacy policy URL: `https://vynl.in/legal/extension-privacy`
- Reviewer notes: explain that `host_permissions` for `app.vynl.in` is used only to read the Clerk session token from the user's own authenticated Vynl tab — no data is read from other origins
